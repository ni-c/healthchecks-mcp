  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      suppressedByReadOnly
        ? 'HEALTHCHECKS_ALLOW_TOOLS selects only write tools, but HEALTHCHECKS_READ_ONLY is ' +
            'set — the server would start with an empty tool list.'
        : 'HEALTHCHECKS_ALLOW_TOOLS/HEALTHCHECKS_DENY_TOOLS leave no tools registered — the ' +
